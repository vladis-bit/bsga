...
const AkademiaNewsSlider = () => {
  ...
  return <div className="relative">
      <div className="relative min-h-[180px] sm:min-h-[200px] md:min-h-[220px] flex items-center justify-center">
        {slides.map((slide, index) => <div key={index} className={`absolute inset-0 flex flex-col items-center justify-center text-center transition-opacity duration-500 ${index === currentSlide ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
      ...
      <div className="flex justify-center gap-2 mt-6">
        {slides.map((_, index) => <button key={index} onClick={() => goToSlide(index)} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentSlide ? "bg-gold w-6" : "bg-primary-foreground/30 hover:bg-primary-foreground/50"}`} aria-label={`Slide ${index + 1}`} />)}
      </div>
    </div>;
};
