  ...
  return <section className="relative h-screen w-full bg-background pt-16 sm:pt-20">
      <div className="absolute inset-0 top-16 sm:top-20 p-2 sm:p-4 md:p-8">
        <div className="relative w-full h-full rounded-2xl sm:rounded-3xl overflow-hidden">
          {slides.map((slide, index) => <div key={index} className={cn("absolute inset-0 transition-opacity duration-1000", index === currentSlide ? "opacity-100" : "opacity-0")}>
          ...
          {/* Navigation Arrows */}
          <button onClick={prevSlide} className="absolute left-2 sm:left-6 md:left-12 top-1/2 -translate-y-1/2 p-2 sm:p-3 text-primary-foreground/70 hover:text-primary-foreground transition-colors" aria-label="Previous slide">
            
          </button>
          <button onClick={nextSlide} className="absolute right-2 sm:right-6 md:right-12 top-1/2 -translate-y-1/2 p-2 sm:p-3 text-primary-foreground/70 hover:text-primary-foreground transition-colors" aria-label="Next slide">
            
          </button>

          {/* Dots */}
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3">
            {slides.map((_, index) => <button key={index} onClick={() => goToSlide(index)} className={cn("w-3 h-3 rounded-full transition-all duration-300", index === currentSlide ? "bg-gold w-8" : "bg-primary-foreground/40 hover:bg-primary-foreground/60")} aria-label={`Go to slide ${index + 1}`} />)}
          </div>
        </div>
      </div>
    </section>;
